...
{  
    ...
    std::wcout<<"PngInit("<<nWidth<<", "<<nHeight<<", "<<psc->m_bGrayscale<<", "<<sFileName+"aaa"<<")="<<bInit<<";\n";
    std::cout<<"Test\n";
        if(!bInit)
            goto cleanup;
    }
...
void CScreenCapture::GetScreenRect(LPRECT rcScreen)
{
    ...
    rcScreen->left   = MulDiv(nLeft          ,Dpi ,96);
    rcScreen->top    = MulDiv(nTop           ,Dpi ,96);
    rcScreen->right  = MulDiv(nLeft +nWidth  ,Dpi ,96);
    rcScreen->bottom = MulDiv(nTop  +nHeight ,Dpi ,96);
}
    ...
    if(IsWindowVisible(hWnd)) // Get only wisible windows
    {
        ...
        {     
            ...
            if((dwStyle&WS_CHILD)==0) // Get only non-child windows
            {
                ...
                wi.m_rcWnd.left   = MulDiv(wi.m_rcWnd.left   ,Dpi ,96);
                wi.m_rcWnd.top    = MulDiv(wi.m_rcWnd.top    ,Dpi ,96);
                wi.m_rcWnd.right  = MulDiv(wi.m_rcWnd.right  ,Dpi ,96);
                wi.m_rcWnd.bottom = MulDiv(wi.m_rcWnd.bottom ,Dpi ,96);
